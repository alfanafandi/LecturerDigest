...
class _AskAiChatState extends State<AskAiChat> with WidgetsBindingObserver {
  ...
  Future<void> _handleAttachFile() async {
    ...
    if (lecture == null && course == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Silakan pilih konteks materi atau kelas terlebih dahulu.'),
  ...
  Widget build(BuildContext context) {
    return Consumer<AppProvider>(
      builder: (context, provider, child) {
        ...
        if (messages.length != _lastMessageCount || isLoading != _lastIsLoading) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: [
              IconButton(
                icon: const Icon(Icons.add_comment_rounded, color: AppTheme.primary),
                onPressed: () {
                    ...
                    : Stack(
                        ...
                        children: [
                          ListView(
                            ...
                            children: [
                              _buildSleekContextBadge(context, provider, lecture, course),
                              ...
                              ...messages.map((m) {
                                  if (m['role'] == 'bot') {
                                    ...
                                    return Padding(
                                      padding: const EdgeInsets.only(bottom: 36.0),
                                      child: _buildUserMessage(context, m['text']),
                                    );
                                  }
                                }),
                        ...
                        SingleChildScrollView(
                          ...
                          child: Row(
                            children: provider.activeRemedialPrompt != null
                                ? [
                                    _buildSuggestionChip('Jelaskan materi yang saya salah', () => provider.sendChatMessage('Berdasarkan hasil kuis kemarin, tolong jelaskan bagian konsep yang masih saya salah.')),
                                    const SizedBox(width: 8),
                                    _buildSuggestionChip('Berikan latihan soal sederhana', () => provider.sendChatMessage('Tolong berikan saya 1 contoh soal latihan sederhana untuk melatih pemahaman konsep tersebut.')),
                                    const SizedBox(width: 8),
                                    _buildSuggestionChip('Bagaimana cara memperbaikinya?', () => provider.sendChatMessage('Apa langkah atau metode belajar terbaik agar saya paham konsep ini?')),
                                  ]
                                : [
                            ...
                            child: Row(
                              children: [
                                Container(
                                  ...
                                  child: Icon(
                                    _isExtractingFile ? Icons.sync : Icons.picture_as_pdf_rounded,
                                    color: AppTheme.primary,
                                    size: 20,
                                  ),
                                ),
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        ...
                                        style: TextStyle(
                                          ...
                                          color: _isExtractingFile ? AppTheme.primary : AppTheme.onSurfaceVariant,
                                        ),
                                      ),
                                    ],
                                  ),
  ...
  Widget _buildSleekContextBadge(BuildContext context, AppProvider provider, Map<String, dynamic>? lecture, Map<String, dynamic>? course) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: [
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(
                    lecture != null ? Icons.description_outlined : Icons.school_rounded,
                    color: AppTheme.primary,
                    size: 18,
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        lecture != null ? 'DISKUSI MATERI AKTIF' : 'DISKUSI KELAS AKTIF',
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Ganti', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
              ...
              SnackBar(
                content: const Row(
                  children: [
                    ...
                    SizedBox(width: 10),
                    Text('Jawaban disalin ke clipboard!', style: TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
  ...
  Widget _buildActionIconButton({required IconData icon, required String tooltip, required VoidCallback onTap}) {
    return Material(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.forum_outlined, size: 40, color: AppTheme.primary),
            ),
            const SizedBox(height: 24),
            Text(
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Pilih Konteks Belajar', style: TextStyle(fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(
                backgroundColor: AppTheme.primary,
              ...
              ...provider.recentChatSessions.take(3).map((session) {
                return Card(
                  ...
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(16),
                    side: BorderSide(color: AppTheme.outlineVariant.withOpacity(0.12)),
                  ),
                  color: AppTheme.surfaceContainerLow,
                  child: ListTile(
            ...
            ? GridView.count(
                ...
                children: [
                  _buildPromptCard(
                    ...
                    () => _sendSuggestionPrompt(provider, 'Uraikan teori utama dan rumusan inti dari materi ini secara sederhana.'),
                  ),
                  _buildPromptCard(
                    Icons.lightbulb_outline_rounded,
                    'Buat Ringkasan Eksekutif',
                    'Tolong buatkan ringkasan singkat tapi padat dalam 5 poin penting.',
                    () => _sendSuggestionPrompt(provider, 'Tolong buatkan ringkasan singkat tapi padat dalam 5 poin penting.'),
                  ),
                  _buildPromptCard(
                    Icons.quiz_outlined,
                    'Latihan Soal & Jawaban',
                    'Berikan 3 pertanyaan kritis beserta jawabannya untuk latihan ujian.',
                    () => _sendSuggestionPrompt(provider, 'Berikan 3 pertanyaan kritis beserta jawabannya untuk latihan ujian.'),
                  ),
                  _buildPromptCard(
                    Icons.history_edu_rounded,
                    'Hubungan dengan Praktek',
                    'Bagaimana materi teori ini diaplikasikan pada dunia kerja riil?',
                    () => _sendSuggestionPrompt(provider, 'Bagaimana materi teori ini diaplikasikan pada dunia kerja riil?'),
                  ),
                ],
              )
            : Column(
                children: [
                  _buildPromptCard(
                    ...
                    () => _sendSuggestionPrompt(provider, 'Uraikan teori utama dari materi ini secara sederhana.'),
                  ),
                  const SizedBox(height: 12),
                  _buildPromptCard(
                    Icons.lightbulb_outline_rounded,
                    ...
                    () => _sendSuggestionPrompt(provider, 'Tolong buatkan ringkasan singkat dalam 5 poin penting.'),
                  ),
                ],
              ),
          ],
  ...
  Widget _buildPromptCard(IconData icon, String title, String body, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
  ...
  void _sendSuggestionPrompt(AppProvider provider, String prompt) {
    ...
    if (lecture == null && course == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Silakan pilih konteks materi/kelas terlebih dahulu di tombol atas.')),
      );
    } else {
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Riwayat Obrolan AI', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
                  IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close)),
                ],
              ),
            ),
            ...
            Expanded(
              child: provider.recentChatSessions.isEmpty
                  ? const Center(
                      child: Text('Belum ada riwayat percakapan.', style: TextStyle(color: AppTheme.onSurfaceVariant)),
                    )
                  : ListView.builder(
                      padding: const EdgeInsets.all(16),
